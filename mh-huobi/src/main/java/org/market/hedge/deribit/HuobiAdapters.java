...
import org.market.hedge.deribit.dto.account.HuobiBalanceRecord;
import org.market.hedge.deribit.dto.account.HuobiBalanceSum;
import org.market.hedge.deribit.dto.account.HuobiFundingRecord;
...
import org.market.hedge.deribit.dto.trade.HuobiOrder;
...
public class HuobiAdapters {
  ...
  private static UserTrade adaptTrade(LimitOrder order) {
    ...
    return new UserTrade.Builder()
        ...
        .currencyPair((CurrencyPair)order.getInstrument())
        .price(order.getLimitPrice())
        .timestamp(order.getTimestamp())
        .id("") // Trade id
        .orderId(order.getId()) // Original order id
        .feeAmount(feeAmount)
        .feeCurrency(((CurrencyPair)order.getInstrument()).counter)
        .build();
  }
